...
HTML reports include 10 specialized recipe-specific renderers covering per-lane BER with confidence intervals, eye scan margins, bandwidth per-port charts, LTSSM state transitions, Gen6 capability checklists, and Flit performance metrics. A generic renderer handles all remaining recipes and includes full `measured_values` data. Reports display device identification (chip type, silicon revision, BDF address) in headers, test parameters per recipe, wall-clock timestamps on step results, pass/fail threshold documentation, and print-friendly CSS with page break handling. CSV export includes a `measured_values_json` column with full measurement data alongside individual scalar columns, a `device_id` field in the summary CSV, and CSV injection protection.
