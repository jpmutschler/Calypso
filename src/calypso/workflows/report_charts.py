...
def status_color(status_value: str) -> str:
    ...
    return _STATUS_COLORS.get(status_value, _TEXT_MUTED)
...
def metric_card(label: str, value: str, color: str = _TEXT_PRIMARY) -> str:
    ...
    return (
        ...
        f'<div style="font-size:11px; color:{_TEXT_MUTED}; margin-top:4px;">{html.escape(label)}</div>'
        f"</div>"
    )
...
def bar_chart(
    ...
    for label, value in data:
        ...
        rows.append(
            ...
            f'<div style="width:120px; font-size:12px; color:{_TEXT_SECONDARY}; '
            f'text-align:right; padding-right:8px; white-space:nowrap;">{html.escape(str(label))}</div>'
            f'<div style="flex:1; background:{_BG_ELEVATED}; border-radius:4px; '
            ...
            f"</div>"
            f'<div style="width:80px; font-size:12px; color:{_TEXT_PRIMARY}; '
...
def results_table(
    ...
    """
    header_cells = "".join(
        f'<th style="text-align:left; padding:8px 12px; color:{_TEXT_SECONDARY}; '
        f'font-size:12px; font-weight:600; border-bottom:1px solid {_BORDER};">'
    ...
    for row in rows:
        ...
        for i, cell in enumerate(row):
            ...
            cells.append(
                ...
                f'font-size:13px; color:{_TEXT_PRIMARY};">{cell_html}</td>'
            )
        body_rows.append(f"<tr>{''.join(cells)}</tr>")

    return (
        f'<table style="width:100%; border-collapse:collapse; '
        f'background:{_BG_CARD}; border-radius:8px; overflow:hidden;">'
...
def section_header(title: str, subtitle: str = "") -> str:
    ...
    sub = (
        f'<div style="font-size:13px; color:{_TEXT_MUTED}; margin-top:4px;">'
    ...
    return (
        ...
        f'<div style="font-size:18px; font-weight:600; color:{_TEXT_PRIMARY};">'
