...
from calypso.models.ltssm import LTSSM_STATE_CATEGORY, LtssmState
...
def _state_color(state_code: int) -> str:
    """Return a display color for an LTSSM state code."""
    for category, codes in LTSSM_STATE_CATEGORY.items():
        if state_code in codes:
            return {
                "Detect": COLORS.red,
                "Polling": COLORS.orange,
                "Configuration": COLORS.yellow,
                "Recovery": COLORS.blue,
                "L0": COLORS.green,
                "L0s": COLORS.cyan,
                "L1": COLORS.purple,
                "L2": COLORS.text_muted,
                "Disabled": COLORS.red,
                "Loopback": COLORS.orange,
                "Hot Reset": COLORS.red,
            }.get(category, COLORS.text_secondary)
...
def _ltssm_trace_content(device_id: str) -> None:
    ...
    def _update_snapshot_display(data: dict):
        ...
        with snapshot_container:
            ...
            with ui.row().classes("w-full gap-8 items-start flex-wrap"):
                _snapshot_stat(
                    "LTSSM State", ltssm_name,
                    f"0x{ltssm_code:02X}", color,
                )
                _snapshot_stat(
        ...
        with retrain_summary:
            with ui.row().classes("w-full gap-8 items-start flex-wrap"):
                _snapshot_stat(
                    ...
                    f"0x{data.get('final_state', 0):02X}",
            ...
            rows = [
                {
                    ...
                    "state_code": f"0x{t['state']:02X}",
                    "state_name": t["state_name"],
                }
                for t in transitions
            ]
            columns = [
                {"name": "timestamp_ms", "label": "Timestamp (ms)", "field": "timestamp_ms", "align": "left"},
                {"name": "state_code", "label": "State Code", "field": "state_code", "align": "left"},
                {"name": "state_name", "label": "State Name", "field": "state_name", "align": "left"},
            ]
            ui.table(
                columns=columns,
                rows=rows,
    ...
    def _render_ptrace_data(data: dict):
        ...
        with ptrace_data_container:
            if not entries:
                ...
                return
            rows = [
                {"index": str(e["index"]), "raw_data": e["raw_data"]}
                for e in entries
            ]
            columns = [
                {"name": "index", "label": "Index", "field": "index", "align": "left"},
                {"name": "raw_data", "label": "Raw Data (hex)", "field": "raw_data", "align": "left"},
            ]
    ...
    with ui.card().classes("w-full p-4").style(
        ...
        ui.label("LTSSM Trace Controls").classes("text-h6").style(
        ...
        with ui.row().classes("items-end gap-4 mt-2"):
            ...
            ui.button("Retrain & Watch", on_click=start_retrain).props(
                "flat color=warning"
            )
            ui.button("Clear Counters", on_click=clear_counters).props(
                "flat color=negative"
            )
    ...
    snapshot_card = ui.card().classes("w-full p-4").style(
        f"background: {COLORS.bg_secondary}; border: 1px solid {COLORS.border}"
    )
    snapshot_card.set_visibility(False)
    with snapshot_card:
        ui.label("Current State").classes("text-h6").style(
            f"color: {COLORS.text_primary};"
        )
        snapshot_container = ui.row().classes("w-full mt-2")

    # --- Retrain progress card ---
    retrain_progress_card = ui.card().classes("w-full p-4").style(
        f"background: {COLORS.bg_secondary}; border: 1px solid {COLORS.border}"
    )
    ...
    # --- Retrain chart card ---
    retrain_chart_card = ui.card().classes("w-full p-4").style(
        f"background: {COLORS.bg_secondary}; border: 1px solid {COLORS.border}"
    )
    ...
    transition_table_card = ui.card().classes("w-full p-4").style(
        f"background: {COLORS.bg_secondary}; border: 1px solid {COLORS.border}"
    )
    transition_table_card.set_visibility(False)
    with transition_table_card:
        ui.label("Transition Log").classes("text-h6").style(
            f"color: {COLORS.text_primary};"
        )
    ...
    with ui.card().classes("w-full p-4").style(
        ...
        ui.label("Ptrace Capture").classes("text-h6").style(
            f"color: {COLORS.text_primary};"
        )
        with ui.row().classes("items-end gap-4 mt-2"):
            ptrace_trace_point = ui.number(
                "Trace Point", value=0, min=0, max=15, step=1,
            ).props("dense outlined").classes("w-28")

            ptrace_lane = ui.number(
                "Lane Select", value=0, min=0, max=15, step=1,
            ).props("dense outlined").classes("w-28")

            ptrace_trigger_toggle = ui.switch("LTSSM Trigger").props("dense")

            # Build LTSSM state options for dropdown
            ltssm_options = {s.value: s.name for s in LtssmState}
            ptrace_trigger_state = ui.select(
                ltssm_options,
                label="Trigger State",
                value=0x0F,
    ...
    # --- Ptrace data card ---
    ptrace_data_card = ui.card().classes("w-full p-4").style(
        f"background: {COLORS.bg_secondary}; border: 1px solid {COLORS.border}"
    )
    ptrace_data_card.set_visibility(False)
    with ptrace_data_card:
        ui.label("Ptrace Captured Data").classes("text-h6").style(
            f"color: {COLORS.text_primary};"
        )
...
def _get_state_label(code: int) -> str:
    """Get a short label for a state code."""
    try:
        name = LtssmState(code).name
        # Shorten common prefixes for chart readability
        for prefix in ("CONFIG_", "RECOVERY_", "POLLING_", "DETECT_", "LOOPBACK_", "L0S_"):
