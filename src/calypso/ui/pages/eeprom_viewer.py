...
def eeprom_viewer_page(device_id: str) -> None:
    ...
    def content():
        _eeprom_viewer_content(device_id)

    page_layout("EEPROM Viewer", content, device_id=device_id)


def _eeprom_viewer_content(device_id: str) -> None:
    """Build the EEPROM viewer page content."""
            ...
            with info_container:
                ...
                with ui.row().classes("gap-6"):
                    _status_indicator(
                        "Present",
                        str(present),
                        COLORS.green if present else COLORS.red,
                    )
                    _status_indicator(
                        "Status",
                        status,
                        COLORS.green if status == "valid"
                        else COLORS.red if status == "invalid"
                    ...
                    _status_indicator(
                        "CRC Status",
                        crc_status,
                        COLORS.green if crc_status == "valid"
                        else COLORS.red if crc_status == "invalid"
